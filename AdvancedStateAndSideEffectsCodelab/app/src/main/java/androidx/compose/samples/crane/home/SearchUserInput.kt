...
fun ToDestinationUserInput(onToDestinationChanged: (String) -> Unit) {
    ...
    // This let us call onToDestinationChanged lambda everytime editableUserInputState changes
    val currentOnDestinationChanged by rememberUpdatedState(onToDestinationChanged)
    LaunchedEffect(editableUserInputState) { // When the state read inside snapshotFlow mutates, the Flow will emit the new value to the collector.
        snapshotFlow { editableUserInputState.text } // State<T> to Flow<T>
            .filter { !editableUserInputState.isHint } // Flow<T>.filter
            .collect {
